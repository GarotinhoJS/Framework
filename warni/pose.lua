...
AddEventHandler("projectEZ:savelastpos", function( LastPosX , LastPosY , LastPosZ , LastPosH )
	TriggerEvent('es:getPlayerFromId', source, function(user)
		...
		warni.execute("UPDATE users SET `lastpos`=@lastpos WHERE identifier = @username", {['@username'] = player, ['@lastpos'] = LastPos})
	end)
end)
...
AddEventHandler("projectEZ:SpawnPlayer", function()
	TriggerEvent('es:getPlayerFromId', source, function(user)
		...
		local result = warni.fetchScalar("SELECT lastpos FROM users WHERE identifier = @username", {['@username'] = player})	
